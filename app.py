...
# --- Боковая панель ---
with st.sidebar:
	st.header(t.get('params', 'Параметры задачи'))
	kpi = st.text_area(t.get('kpi', 'KPI (Цель)'),
	                   value="Increase creep resistance of nickel-based superalloy by 20% at 700°C")
	constraints = st.text_area(t.get('constraints', 'Ограничения'),
	                           value="No rhenium, budget < $500k, scalable to industrial production")
	
	st.subheader(t.get('model', 'Настройки модели'))
	
	provider_choice = st.selectbox(
		t.get('provider', 'Провайдер нейросети'),
		["Ollama (Локально)", "YandexGPT (API)", "G4F (Бесплатно)"]
	)
	
	# Маппинг названий из UI в внутренние значения для движка
	provider_map = {
		"Ollama (Локально)": "ollama",
		"YandexGPT (API)": "yandex",
		"G4F (Бесплатно)": "g4f"
	}
	selected_provider = provider_map[provider_choice]
	
	# Динамический выбор модели в зависимости от провайдера
	available_models = {
		"ollama": ["llama3.1:8b", "mistral:7b", "llama3.2:3b"],
		"yandex": ["yandexgpt-lite", "yandexgpt"],
		"g4f": ["gpt-4o-mini", "gpt-4o", "gpt-3.5-turbo"]
	}
	
	model_name = st.selectbox(
		t.get('local_model', 'Выберите модель'),
		available_models[selected_provider],
		index=0
	)
	
	st.caption("Выберите провайдера и конкретную модель. Для Ollama модель должна быть скачана локально.")
		...
		with st.spinner("Идёт генерация гипотез... (может занять 20–60 секунд)"):
			try:
				...
				full_context = graph_context + "\nLiterature insights: " + "; ".join(
					[f"{rel['subject']} {rel['relation']} {rel['object']}" for rel in analysis['relationships'][:5]])
...
if 'hypotheses' in st.session_state:
	...
	with col1:
		if st.button(t['export_json']):
			...
			st.download_button(label="Скачать JSON", data=json_str,
	...
	with col2:
		if st.button(t['export_txt']):
			...
			st.download_button(label="Скачать TXT", data=report,
